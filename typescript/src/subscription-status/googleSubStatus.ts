...
import type { HttpRequestHeaders, PathParameters } from '../models/apiGatewayHttp';
...
async function updateParallelTestTable(purchaseToken: string, packageName: string) {
    try {
        const googleSubscription = await fetchGoogleSubscriptionV2(purchaseToken, packageName);
