...
export type AppleStoreKitSubscriptionDataDerivationForExtra =
...
const transactionIdToAppleStoreKitSubscriptionData = async (
    ...
    */
	...
	const data1 = jwt.decode(
		signedTransactionInfo,
	) as AppleStoreKitSubscriptionData;
...
const appleSubscriptionToOriginalTransactionId = (
...
export const appleSubscriptionToAppleStoreKitSubscriptionDataDerivationForFeastPipeline =
	async (
		subscription: Subscription,
	): Promise<
  ...
  */
		...
		const transactionId =
			appleSubscriptionToOriginalTransactionId(subscription);
		...
		console.log(
			`[52c0e2ef] appBundleId: ${appBundleId}, transactionId: ${transactionId}`,
		);
		...
		if (appleStoreKitSubscriptionData === undefined) {
			console.log(
				`[b52fe205] could not retrieve appleStoreKitSubscriptionData`,
			);
			return Promise.resolve(undefined);
		}
		...
		const country = storefrontToCountry(
			appleStoreKitSubscriptionData.storefront,
		);
		const currency = appleStoreKitSubscriptionData.currency;
		const paymentFrequency = productIdToPaymentFrequency(
			appleStoreKitSubscriptionData.productId,
		);
...
export const transactionIdToAppleStoreKitSubscriptionDataDerivationForExtra =
	async (
		appBundleId: string,
		transactionId: string,
		...
		console.log(
			`[e2b0930d] appBundleId: ${appBundleId}, transactionId: ${transactionId}`,
		);
