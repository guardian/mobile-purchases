...
const extractOfferTagsFromSubscription = (
	subscription: E1GoogleSubscription,
): string[] => {
...
const buildExtraObject = async (
	...
	const subscription = await extractGoogleSubscription(
		accessToken,
		packageName,
		purchaseToken,
	);
...
export async function extraction(
	...
	try {
		...
		const offer_id =
			extractFirstOfferTagFromSubscription(googleSubscription) ?? null;
		const answer = {
			extra: extraObject,
			offer_id,
		};
		...
		if (error instanceof Error) {
			...
			console.error(
				`[26b810d4] error: building failed: ${JSON.stringify(error)}`,
			);
		}
		return Promise.resolve(undefined);
	}
