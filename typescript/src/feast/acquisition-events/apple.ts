...
const appleSubscriptionToAcquisitionApiPayload = async (
  ...
  const extendedData: AppleStoreKitSubscriptionDataDerivationForFeastPipeline | undefined =
    await appleSubscriptionToAppleStoreKitSubscriptionDataDerivationForFeastPipeline(subscription);

  if (extendedData === undefined) {
    console.error(`[e88e8329] there was a problem while acquiring subscription data from Apple`);
    throw new Error('[ae7b14ba] there was a problem while acquiring subscription data from Apple');
  }
