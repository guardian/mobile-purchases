...
async function recursivelyFetchSqsMessages(sqsUrl: string, remainingMessagesToFetch: number, handleMsg: (message: Message) => void): Promise<void> {
    if (remainingMessagesToFetch > 0) {
        ...
        if (sqsResp.Messages && sqsResp.Messages.length > 0) {
            ...
            return await recursivelyFetchSqsMessages(sqsUrl, remainingMessagesToFetch - sqsResp.Messages.length, handleMsg)
        }
    } else {
        console.log("Terminating processing early as we've reached the limit of messages to fetch")
    }
}
...
export async function handler(params: {date: string, maxMessagesToFetch?: number}): Promise<any> {
    ...
    await recursivelyFetchSqsMessages(sqsUrl, maxMessagesToFetch, handleOneMessage);
